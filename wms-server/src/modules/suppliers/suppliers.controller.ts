...
router.get('/', async (req: AuthRequest, res: any) => {
  try {
    ...
    const [count]: any = await pool.query('SELECT COUNT(*) as total FROM suppliers');
...
router.get('/all', async (_req: AuthRequest, res: any) => {
  try {
    const [rows]: any = await pool.query('SELECT id, code, name FROM suppliers WHERE status=1 ORDER BY name');
...
router.post('/', requireRole('admin', 'manager'), async (req: AuthRequest, res: any) => {
  try {
    ...
    await pool.query('INSERT INTO suppliers (code, name, contact_name, phone, address) VALUES (?, ?, ?, ?, ?)', [code, name, contact_name || '', phone || '', address || '']);
    return success(res, null, '供应商创建成功');
  } catch (err: any) {
...
router.put('/:id', requireRole('admin', 'manager'), async (req: AuthRequest, res: any) => {
  try {
    ...
    await pool.query('UPDATE suppliers SET name=?, contact_name=?, phone=?, address=? WHERE id=?', [name, contact_name || '', phone || '', address || '', req.params.id]);
    return success(res, null, '供应商更新成功');
  } catch (err: any) { return fail(res, err.message); }
});

router.put('/:id/status', requireRole('admin', 'manager'), async (req: AuthRequest, res: any) => {
  await pool.query('UPDATE suppliers SET status=? WHERE id=?', [req.body.status, req.params.id]);
