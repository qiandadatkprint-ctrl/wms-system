<template>
    ...
    <template #header>
      <div class="card-header">
        ...
        <div>
          ...
          <el-button type="primary" @click="open()" v-if="isManager">新增客户</el-button>
        </div>
      </div>
    </template>
  ...
  <el-dialog v-model="show" :title="editing?'编辑客户':'新增客户'" width="500px">
...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/customers', { params: { page: page.value, pageSize: pageSize.value, keyword: keyword.value } })
...
async function save() {
  if (editing.value) await api.put(`/customers/${(form as any).id}`, form)
  else await api.post('/customers', form)
  ElMessage.success('保存成功'); show.value = false; load()
}
async function toggle(row: any) {
  await api.put(`/customers/${row.id}/status`, { status: row.status === 1 ? 0 : 1 })
